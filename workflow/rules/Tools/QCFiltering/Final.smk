...
use rule create_local_links as create_final_track_fastq_links with:
    input:
        fastq=lambda wildcards: config["out_dir"]/ ("ext_track_data/%s/%s/%s/%s%s" % (wildcards.datatype, wildcards.track_name,
                                                                                  "filtered" if wildcards.datatype in config["data_feature_dict"]["filter"] else "raw",
                                                                                  wildcards.fileprefix,
                                                                                  config["fastq_ext"])),
        log_dir=ancient(config["out_dir"] / "log/")
    output:
        fastq=config["out_dir"] / ("ext_track_data/{datatype}/{track_name}/final/{fileprefix}%s" % config["fastq_ext"])
    log:
...
use rule create_local_links as create_final_track_fasta_links with:
    input:
        ...
        log_dir=ancient(config["out_dir"] / "log/")
    output:
        fastq=config["out_dir"] / ("ext_track_data/{datatype}/{track_name}/final/{fileprefix}%s" % config["fasta_ext"])
    log:
