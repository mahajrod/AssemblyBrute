...
rule pretext_inject_tracks:
    ...
    params:
        ...
        purge_dups_datatype_list=parameters["tool_options"]["assembly_qc"]["purge_dups"]["datatype_list"],
    log:
    ...
    shell:
        ...
        "               if [[ -s ${{BUSCO_TRACK}} ]]; "
        "                   then "
        "                   echo \"Preprocessing ${{BUSCO_TRACK}}...\" >> {log.preprocessing}; "
        "                   workflow/scripts/curation/filter_bed_by_scaffolds.py -d {input.filtered_out} -i ${{BUSCO_TRACK}} > ${{BUSCO_TRACK}}.{wildcards.subset}.tmp 2>>{log.preprocessing}; "
        "                   if [[ -s ${{BUSCO_TRACK}}.{wildcards.subset}.tmp ]]; "
        "                   then "
        "                       echo \"Injecting ${{BUSCO_TRACK}}.{wildcards.subset}.tmp...\" >> {log.injection}; "
        "                       cat ${{BUSCO_TRACK}}.{wildcards.subset}.tmp | PretextGraph -i {output.updated_map} -n BUSCO.${{LINEAGE}}.${{BUSCO_TYPE}}.gap >> {log.injection} 2>&1; "
        "                   fi;"
        "               fi; "
        "           done; "
        "      done; "
        ...
        "           then "
        "               echo \"Preprocessing ${{PURGE_DUPS_TRACK}}...\" >> {log.preprocessing}; "
        "               workflow/scripts/curation/filter_bed_by_scaffolds.py -d {input.filtered_out} -i ${{PURGE_DUPS_TRACK}} > ${{PURGE_DUPS_TRACK}}.{wildcards.subset}.tmp 2>>{log.preprocessing}; "
        "               if [[ -s ${{PURGE_DUPS_TRACK}}.{wildcards.subset}.tmp ]]; "
        "               then "
        "                   echo \"Injecting ${{PURGE_DUPS_TRACK}}.{wildcards.subset}.tmp...\" >> {log.injection}; "
        "                   cat ${{PURGE_DUPS_TRACK}}.{wildcards.subset}.tmp  | PretextGraph -i {output.updated_map} -n PURGE_DUPS.${{DATATYPE}}.${{ARTEFACT_TYPE}}.gap >> {log.injection} 2>&1; "
        "               fi; "
        "          fi; "
        "      done; "
        "   done; "
