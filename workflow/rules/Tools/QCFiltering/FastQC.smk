...
use rule fastqc as fastqc_track_data with:
    input:
        fastq=config["out_dir"] / ("track_data/{fastqc_datatype}/{track_name}/{stage}/{fileprefix}%s" % config["fastq_ext"])
    output:
        zip=config["out_dir"] / "track_qc/fastqc/{fastqc_datatype}/{track_name}/{stage}/{fileprefix}_fastqc.zip"
    log:
