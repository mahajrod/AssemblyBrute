...
rule pretext_inject_tracks:
    ...
    shell:
        ...
        "       workflow/scripts/curation/filter_bed_by_scaffolds.py -d {input.filtered_out} -i ${{TRACK}} > ${{TRACK}}.tmp 2>>{log.preprocessing}; " 
        "   fi;"
        " done; "
        " "
        ...
        "   then "
        "   echo \"Injecting {input.canonical_telomere_tidk_track}.tmp...\" >> {log.injection}; "
        "   cat {input.canonical_telomere_tidk_track}.tmp | PretextGraph -i {output.updated_map} -n canonical_tidk.telomere >> {log.injection} 2>&1; "
        "   fi; "
        " if [[ -s {input.non_canonical_telomere_tidk_track}.tmp ]]; "
        "   then "
        "   echo \"Injecting {input.non_canonical_telomere_tidk_track}.tmp...\" >> {log.injection}; "
        "   cat {input.non_canonical_telomere_tidk_track} | PretextGraph -i {output.updated_map} -n noncanonical_tidk.telomere >> {log.injection} 2>&1; "
        "   fi;  "
        " if [[ -s {input.canonical_telomere_track}.tmp ]]; "
        " then "
        "   echo \"Injecting {input.canonical_telomere_track}.tmp...\" >> {log.injection}; "
        "   awk '{{printf \"%s\\t%i\\t%i\\t%i\\n\",$1,$2,$3,$4}}' {input.canonical_telomere_track}.tmp  | "
        "       PretextGraph -i {output.updated_map} -n canonical.telomere >> {log.injection} 2>&1; "
        " fi; "
        " if [[ -s {input.non_canonical_telomere_track}.tmp ]]; "
        "   then "
        "   echo \"Injecting {input.non_canonical_telomere_track}.tmp...\" >> {log.injection}; "
        "   awk '{{printf \"%s\\t%i\\t%i\\t%i\\n\",$1,$2,$3,$4}}' {input.non_canonical_telomere_track}.tmp | "
