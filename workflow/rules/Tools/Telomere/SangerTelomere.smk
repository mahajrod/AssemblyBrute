...
rule telo_finder:
    ...
    output:
        canonical="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.canonical.txt",
        canonical_kmer="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.canonical.kmer",
        canonical_top_kmer="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.canonical.top.kmer",
        non_canonical="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.non_canonical.txt",
        non_canonical_kmer="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.non_canonical.kmer",
    ...
    log:
        std="{fasta_dir}/log/telo_finder.{fasta_prefix}.log",
        cluster_log="{fasta_dir}/log/telo_finder.{fasta_prefix}.cluster.log",
        cluster_err="{fasta_dir}/log/telo_finder.{fasta_prefix}.cluster.err"
    benchmark:
        "{fasta_dir}/log/telo_finder.{fasta_prefix}.benchmark.txt"
    conda:
...
rule telo_container: #TODO: add possibility to use custom telomere c
    ...
    output:
        ...
        canonical_telo_bed="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.canonical.telomere.bed",
        canonical_telo="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.canonical.telomere",
        canonical_telo_win="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.canonical.telomere.windows",
        ...
        non_canonical_telo_bed="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.non_canonical.telomere.bed",
        non_canonical_telo="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.non_canonical.telomere",
        non_canonical_telo_win="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.non_canonical.telomere.windows",
    params:
    ...
    log:
        std="{fasta_dir}/log/telo_container.{fasta_prefix}.log",
        cluster_log="{fasta_dir}/log/telo_container.{fasta_prefix}.cluster.log",
        cluster_err="{fasta_dir}/log/telo_container.{fasta_prefix}.cluster.err"
    benchmark:
        "{fasta_dir}/log/telo_container.{fasta_prefix}.benchmark.txt"
    conda:
    ...
    shell: # TODO pack code below as script. Issue with container - it return 111 inside of 0 and snakemake breaks. Added ' || true'
        ...
        "     singularity run --pid --contain {params.container} -t ${{CANONICAL_OUTPUT_PREFIX}} "
        ...
        "     singularity run --pid --contain {params.container} -t ${{NON_CANONICAL_OUTPUT_PREFIX}} "
...
rule collapse_overlapping_telomere_windows:
    ...
    output:
        canonical_collapsed_telo_track="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.canonical_telomere.win1000.step200.track.collapsed.bed",
        non_canonical_collapsed_telo_track="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.non_canonical_telomere.win1000.step200.track.collapsed.bed",
    params:
        container=config["tool_containers"]["rapid_telomere"]
    log:
        std="{fasta_dir}/log/collapse_telomere_track.{fasta_prefix}.log",
        cluster_log="{fasta_dir}/log/collapse_telomere_track.{fasta_prefix}.cluster.log",
        cluster_err="{fasta_dir}/log/collapse_telomere_track.{fasta_prefix}.cluster.err"
    benchmark:
        "{fasta_dir}/log/collapse_telomere_track.{fasta_prefix}.benchmark.txt"
    conda:
        config["conda"]["common"]["name"] if config["use_existing_envs"] else ("../../../%s" % config["conda"]["common"]["yaml"])
    resources:
        queue=config["queue"]["cpu"]["name"],
    ...
    shell:
        ...
        " echo -e '#scaffold\tstart\tend\tlength\tmedian\tmean\tstdev\tmode\tabsmin' > {output.non_canonical_collapsed_telo_track} 2>>{log.std}; "
...
rule classify_telomeric_regions_windows:
    ...
    log:
        std="{fasta_dir}/log/classify_telomeric_regions_windows.{fasta_prefix}.log",
        cluster_log="{fasta_dir}/log/classify_telomeric_regions_windows.{fasta_prefix}.cluster.log",
        cluster_err="{fasta_dir}/log/classify_telomeric_regions_windows.{fasta_prefix}.cluster.err"
    benchmark:
        "{fasta_dir}/log/classify_telomeric_regions_windows.{fasta_prefix}.benchmark.txt"
    conda:
        config["conda"]["common"]["name"] if config["use_existing_envs"] else ("../../../%s" % config["conda"]["common"]["yaml"])
    resources:
        queue=config["queue"]["cpu"]["name"],
...
rule get_telomere_warning:
    ...
    output:
        ...
        canonical_telo_status="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.canonical_telomere_warning.win1000.step200.track.status",
        non_canonical_telo_status="{fasta_dir}/{fasta_prefix}/telomere/{fasta_prefix}.non_canonical_telomere_warning.win1000.step200.track.status"
    params:
        fraction_threshold=parameters["tool_options"]["assembly_qc"]["telomere"]["fraction_threshold"]
    log:
        canonical="{fasta_dir}/log/get_telomere_warning.{fasta_prefix}.canonical.log",
        non_canonical="{fasta_dir}/log/get_telomere_warning.{fasta_prefix}.non_canonical.log",
        cluster_log="{fasta_dir}/log/get_telomere_warning.{fasta_prefix}.cluster.log",
        cluster_err="{fasta_dir}/log/get_telomere_warning.{fasta_prefix}.cluster.err"
    benchmark:
        "{fasta_dir}/log/get_telomere_warning.{fasta_prefix}.benchmark.txt"
    conda:
        config["conda"]["common"]["name"] if config["use_existing_envs"] else ("../../../%s" % config["conda"]["common"]["yaml"])
    resources:
        queue=config["queue"]["cpu"]["name"],
