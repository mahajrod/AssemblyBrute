...
rule place_microsomes_first:
    ...
    output:
        filtered_tsv=out_dir_path / ("{assembly_stage}/{parameters, [^/]+}/{genome_prefix, [^/]+}.{assembly_stage}.combined.max%s.candidates.microchromosomes.filtered.tsv" % parameters["tool_options"]["microsome_detection"]["max_length"]),
