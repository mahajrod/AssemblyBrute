...
rule get_purged_seqs: #
    ...
    output:
        ...
        hapdups=config["out_dir"] / "dedup/{parameters, [^/]*purge_dups[^/]*}/{input_fasta_prefix, [^/]*input[^/]*}/{input_fasta_prefix}/purge_dups/{datatype}/{input_fasta_prefix}.hap.fasta",
    params:
        blacklist_option=lambda wildcards: parse_option("purging_blacklist",
                                                        stage_dict["dedup"].parameters[wildcards.parameters]["option_set"],
    ...
    shell:
        " workflow/scripts/purge_dups/filter_dups_bed.py -i {input.raw_dups_bed} {params.blacklist_option} "
        "         {params.whitelist_option} -o {output.filtered_bed} > {log.filter} 2>&1; "
...
use rule create_local_links as create_final_purge_dups_links with:
    input:
        fasta=lambda wildcards: config["out_dir"] / ("dedup/{parameters}/{genome_prefix}.input.{haplotype}/{genome_prefix}.input.{haplotype}/purge_dups/%s/{genome_prefix}.input.{haplotype}.purged.fasta" % "_".join(stage_dict["dedup"].parameters[wildcards.parameters]["option_set"]["main_datatypes"])),
