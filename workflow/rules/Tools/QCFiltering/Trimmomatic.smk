...
use rule trimmomatic_pe as trimmomatic_pe_track_data with:
    input:
        ...
        reverse_fastq=lambda wildcards: config["out_dir"] / ("ext_data/{0}/{5}/{1}/{2}{3}{4}".format(wildcards.pe_datatype,
                                                                                                       "raw" if wildcards.pe_datatype != "hic" else "trimmed",
                                                                                                       wildcards.pairprefix,
                                                                                                       config["rev_fastq_sfx"],
                                                                                                       config["fastq_ext"],
                                                                                                       wildcards.track_name)),
    output:
        forward_fastq=config["out_dir"] / ("ext_data/{pe_datatype}/{track_name}/filtered/{pairprefix}_1%s" % config["fastq_ext"]),
        forward_se_fastq=config["out_dir"] / ("ext_data/{pe_datatype}/{track_name}/filtered/{pairprefix}_1.se%s" % config["fastq_ext"]),
        reverse_fastq=config["out_dir"] / ("ext_data/{pe_datatype}/{track_name}/filtered/{pairprefix}_2%s" % config["fastq_ext"]),
