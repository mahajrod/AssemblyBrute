...
rule porechop_abi:
    ...
    output:
        trimmed_fastq=output_dict["data"] / ("fastq/{datatype, nanopore|simplex|duplex|ultralong_nanopore}/trimmed/{fileprefix, [^/]+}%s" % config["fastq_extension"]),
    params:
...
rule chopper:
    ...
    output:
        filtered_fastq=output_dict["data"] / ("fastq/{datatype, nanopore|simplex|duplex|ultralong_nanopore}/filtered/{fileprefix, [^/]+}%s" % config["fastq_extension"]),
    params:
