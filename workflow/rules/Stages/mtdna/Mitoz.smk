...
rule downsample_pe_reads:
    input:
        forward_reads=config["out_dir"] / ("data/{datatype}/final/{pairprefix}%s%s" % (config["fastq_ext"])),
        reverse_reads=config["out_dir"] / ("data/{datatype}/final/{pairprefix}%s%s" % (config["fastq_ext"])),
        forward_fastqc=config["out_dir"] / "qc/fastqc/{datatype}/final/{pairprefix}_1_fastqc.zip",
        reverse_fastqc=config["out_dir"] / "qc/fastqc/{datatype}/final/{pairprefix}_2_fastqc.zip",
    output:
...
rule mitoz:
    input:
        forward_reads=lambda wildcards: config["out_dir"] / ("data/%s/final/%s%s%s" % (wildcards.datatype,
                                                                                       wildcards.pairprefix,
                                                                                       config["data"][wildcards.datatype]["conv_fwd_sfx"],
                                                                                       config["data"][wildcards.datatype]["conv_ext"])),
        reverse_reads=lambda wildcards: config["out_dir"] / ("data/%s/final/%s%s%s" % (wildcards.datatype,
                                                                                       wildcards.pairprefix,
        ...
        #reverse_reads=config["out_dir"] / ("data/fastq/{datatype, hic|illumina}/downsampled_mitoz/{pairprefix}_2%s" % config["fastq_ext"]),
    output:
        finish_flag=config["out_dir"] / "mtDNA/mitoz/denovo/{datatype, hic|illumina}/{stage, final}/{pairprefix}/FINISH_FLAG"
    ...
    log:
        mitoz=config["out_dir"] / "log/mitoz.{datatype}.{stage}.{pairprefix}.mitoz.log",
        sed=config["out_dir"] / "log/mitoz.{datatype}.{stage}.{pairprefix}.sed.log",
        cluster_log=config["out_dir"] / "log/mitoz.{datatype}.{stage}.{pairprefix}.cluster.log",
        cluster_err=config["out_dir"] / "log/mitoz.{datatype}.{stage}.{pairprefix}.cluster.err"
    benchmark:
        config["out_dir"] / "log/mitoz.{datatype}.{stage}.{pairprefix}.benchmark.txt"
    conda:
    ...
    shell:
        ...
        " then"
        "     sed 's/^>/>{params.genome_prefix}.{wildcards.pairprefix}_/' "
