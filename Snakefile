...
if ("read_qc" in config["stage_list"]) and (not config["skip_read_qc"]):
    ...
    if ("hic" in data_types) and (config["hic_enzyme_set"] != "OmniC") :
        #print(data_types)
...
if ("filter_reads" in config["stage_list"]) and (not config["skip_filter_reads"]):
    ...
    results += [expand(output_dict["qc"] / "fastqc/{datatype}/{stage}/{fileprefix}_fastqc.zip",
                       datatype=["hic", ],
                       stage=["filtered", ],
                       fileprefix=[*[list(map(lambda s: s + suffix + config["fastq_extension"],
                                           input_pairprefix_dict[datatype])) for suffix in ("_1", "_2")]]
