...
rule gather_nanoplot_stats_per_stage:
    ...
    log:
        std=config["out_dir"] / "log/nanoplot.{longread_datatype}.{stage}.log",
        cluster_log=config["out_dir"] / "log/nanoplot.{longread_datatype}.{stage}.cluster.log",
        cluster_err=config["out_dir"] / "log/nanoplot.{longread_datatype}.{stage}.cluster.err"
    benchmark:
        config["out_dir"] / "log/nanoplot.{longread_datatype}.{stage}.benchmark.txt"
    conda:
        config["conda"]["nanopore"]["name"] if config["use_existing_envs"] else ("../../../%s" % config["conda"]["nanopore"]["yaml"])
    resources:
        queue=config["queue"]["cpu"]["name"],
